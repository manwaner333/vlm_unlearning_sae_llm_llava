...
def get_model_activations(model, inputs, cfg):
    ...
    activations = activations[:,574:575,:]
...
#     forget_biography = "Please describe this figure."
...
#     retain_biography = "Please describe this figure."
...
def extratc_forget_sae_feature_info(forget_dataset, k, hook_name):
    ...
    for forget_index in range(len(forget_dataset)):
        ...
        forget_summed_sae_activations = forget_sae_activations[0].mean(dim=0, keepdim=True) 
        forget_values_ele, forget_indices_ele = torch.topk(forget_summed_sae_activations, k, dim=1)
        values_list.append(forget_values_ele)
        indices_list.append(forget_indices_ele)
    
    values_res = torch.cat(values_list, dim=0)
    indices_res = torch.cat(indices_list, dim=0)

    return values_res, indices_res



def extratc_retain_sae_feature_info(retain_dataset, k, hook_name):

    values_list = []
    indices_list = []

    for retain_index in range(len(retain_dataset)):
        retain_image = retain_dataset[retain_index]['image']
        # retain_biography = retain_dataset[retain_index]['biography']
        retain_biography = "Please describe this figure."
        retain_conversation = conversation_form(retain_biography)
        ...
        retain_summed_sae_activations = retain_sae_activations[0].mean(dim=0, keepdim=True) 
        retain_values_ele, retain_indices_ele = torch.topk(retain_summed_sae_activations, k, dim=1)
        values_list.append(retain_values_ele)
        indices_list.append(retain_indices_ele)
...
forget_values_ele, forget_indices_ele = extratc_forget_sae_feature_info(forget_dataset, k, hook_name) 
retain_values_ele, retain_indices_ele = extratc_retain_sae_feature_info(retain_dataset, k, hook_name)
# forget_values.extend(forget_values_ele.view(1, -1).cpu().numpy().tolist())
# forget_indices.extend(forget_indices_ele.view(1, -1).cpu().numpy().tolist())
# retain_values.extend(retain_values_ele.view(1, -1).cpu().numpy().tolist())
# retain_indices.extend(retain_indices_ele.view(1, -1).cpu().numpy().tolist())

forget_values = forget_values_ele.view(1, -1).cpu().numpy().tolist()
forget_indices = forget_indices_ele.view(1, -1).cpu().numpy().tolist()
retain_values = retain_values_ele.view(1, -1).cpu().numpy().tolist()
retain_indices = retain_indices_ele.view(1, -1).cpu().numpy().tolist()
...
for idx, val in zip(forget_indices[0], forget_values[0]):
...
for idx, val in zip(retain_indices[0], retain_values[0]):
